...
const ThirdParty = () => {
  ...
  const details = useForm({
    initialValues: {
      ...
      email: "",
      website: "",
      type:"RECRUITER_PARTY"
    },
    validate: {
      name: (value) => (value.length <= 1 ? "Party name cannot be empty" : null),
  ...
  const handleFormSubmit = useCallback(
    async (values: FormType) => {
      try {
        setIsLoading(true);
        await sendRecuuiterData({ ...values }, token);
  ...
  const handleDelete = async (candToken: number) => {
    if (window.confirm("It will delete all the related information of party data, Are you sure you want delete the record?") === true) {
      try {
  ...
  return (
    <Container>
      <Title order={1} className={styles.mtop}> Third Party Data</Title>
          ...
          <table className={styles.members_table}>
            <thead>
              <tr>
                ...
                <th>Email </th>
                <th className={styles.hideColl__xs}>Website Link</th>
                <th>Delete</th>
              </tr>
            </thead>
            <tbody>
