...
const GetReferralResults = () => {
  ...
  const [maskResume, setMaskResume] = useState('Masked Resume');
  ...
  const getAllResults = React.useCallback(async () => {
    ...
    try {
      ...
      // let someDate = new Date('2023-09-05T16:15:35.000+00:00');
  ...
  const getAllResultsONSelection = (e: any, candidate: Candidatedata) => {
    ...
    if (e === "Get Response") {
      ...
      window.open(
        `/interview-app/shared-candidate/getAudioResults?token=${canToken}`,
        "_blank"
      );
  ...
  return (
    <Container>
      <Title>Add/Get Interviews</Title>
      <Button
        type="submit"
        variant="filled"
        my="sm"
      ...
      {showEmployer && (
        <div>
            ...
            <table className={styles.members_table}>
              <thead>
                <tr>
                  ...
                  <th className={styles.hideCol__xs}>Linked CV</th>
                  {/* <th>Token</th> */}
                ...
                {response.data !== null &&
                  ...
                  response.data.map((member) => (
                    ...
                    <td><button type='button' className={styles.delete_btn} onClick={() => { handleDelete(member.token); }}>
                      ...
                      <td>
                        <Select
                          ...
                          data={[
                            ...
                            {
                              value: "Send Reminder Interview",
                              label: "Send Reminder Interview",
                            },
                            {
                              value: member.resumeId ? "Share Interview" : '',
                              label: member.resumeId ? "Share Interview" : '',
                            },
                      ...
                      withAsterisk
                    /></td> */}
                      {/* {(member.notes) ? <td id={'note' + member.id} onClick={saveEditNotes}>{member.notes} <span>{<FaRegEdit />}</span></td> :
                        <td id={'editnote' + member.id} > <Textarea
                          placeholder="add notes"
                          onChange={(e) => addNotes(e, member)}

                        /></td>
                      } */}
                      {(member.notes) ? <td id={'note' + member.id}>{member.notes} <span onClick={toggleAddNotesModal}>{<FaRegEdit />}</span></td> :
                        <td id={'editnote' + member.id} >

                        Add Notes<span  onClick={toggleAddNotesModal}>{<FaRegEdit />}</span>
                        </td>
                      }
                    </tr>
                  ))}
          ...
          >
            <div className={styles.edit_note}>Edit Notes </div>
                     <textarea className={styles.edit_textarea} placeholder="Notes" onChange={(e) => getNotesContent(e)}>{updateNotes} </textarea>
                    <div>test</div>
                ...
                >
                  <>
                    ...
                    <Radio.Group
                      ...
                      withAsterisk
                    >
                      <Radio value="showResume" label="Masked Resume" />
                      <Radio value="showOriginResume" label="Original Resume" />
                      <Radio value="NoResume" label="No Response" />
                    </Radio.Group>
                    <Radio.Group
                    ...
                    >
                      ...
                      <Radio value="NoResponse" label="No Resume" />
                    </Radio.Group>
          ...
          <form onSubmit={details.onSubmit(handleFormSubmit)}>
            ...
            */}
              ...
              <Select
                ...
                onSearchChange={(search) => {
                  debounceFetchThirdParties(search, "REFERRAL");
                }}
              />
...
export default GetReferralResults;
